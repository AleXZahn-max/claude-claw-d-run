...
class Clawd {
    ...
    step(speed, dt = 1) {
        ...
        if (this.faceHold === 0 && !this.thinking && this.grounded) {
            if (--this.blinkIn <= 0) {
                ...
                this.blinkIn = 120 + Math.floor(Math.random() * 260);
            }
        }
        ...
        if (this.clawSnap > 0) this.clawSnap--;
            ...
            this.clawSnap = 8;
            this.snapIn = 70 + Math.floor(Math.random() * 130);
        }
    ...
    draw(term, tick) {
        ...
        if (pose === 'dead') {
            term.blit(C.dead, x, y - C.dead.h + 1, { clipAbove: GEO.skyTop });
            return;
        }
        ...
        term.blit(body, x, top, { clipAbove: GEO.skyTop, tint });
        ...
        if (pose === 'run') {
            ...
            term.blit(legs, x, top + body.h, { clipAbove: GEO.skyTop, tint });
            if (this.clawSnap > 0) {
...
class Hazard {
    ...
    draw(term) {
        ...
        term.blit(this.sprite, Math.round(this.x), this.top + bob, { clipAbove: GEO.skyTop });
    }
}
...
class World {
    ...
    seedBackdrop() {
        ...
        for (let i = 0; i < 46; i++) {
            this.stars.push({
                x: Math.random() * GEO.cols,
                y: GEO.skyTop + Math.random() * 11,
                ch: S[Math.floor(Math.random() * S.length)],
                a: 0.25 + Math.random() * 0.5,
            });
        }
        for (let i = 0; i < 5; i++) this.far.push(this.makeDecor(Math.random() * GEO.cols, 'far'));
        for (let i = 0; i < 4; i++) this.mid.push(this.makeDecor(Math.random() * GEO.cols, 'mid'));
    }
    ...
    makeDecor(x, band) {
        if (band === 'far') {
            ...
            return { x, sprite: s, bottom: GEO.skyTop + 6 + Math.floor(Math.random() * 5) };
        }
    ...
    step(speed) {
        ...
        for (const s of this.stars) {
            s.x -= speed * 0.09;
            if (s.x < -1) { s.x += GEO.cols + 2; s.y = GEO.skyTop + Math.random() * 11; }
        }
        for (const d of this.far) {
            d.x -= speed * 0.3;
            if (d.x + d.sprite.w < -2) Object.assign(d, this.makeDecor(GEO.cols + Math.random() * 30, 'far'));
        }
        for (const d of this.mid) {
            d.x -= speed * 0.62;
            if (d.x + d.sprite.w < -2) Object.assign(d, this.makeDecor(GEO.cols + Math.random() * 40, 'mid'));
        }
    }
    ...
    drawSky(term, thinking, tick) {
        ...
        if (thinking) {
            // Thought fragments drift the other way, slower than everything
            // else, so the world visibly stops making sense at normal speed.
            const T = GLYPHS.DECOR.thoughts;
            for (let i = 0; i < 5; i++) {
                const t = T[(i * 3 + Math.floor(tick / 90)) % T.length];
                const x = Math.round(((tick * 0.13 + i * 34) % (GEO.cols + 20)) - 18);
                term.text(x, GEO.skyTop + 1 + i * 2, t, 'cyan', 0.34);
            }
        }
    }
    ...
    drawGround(term) {
        const b = this.biome;
        const off = Math.floor(this.distance);
        ...
        for (let row = GEO.groundRow + 1; row < GEO.statusRow; row++) {
            ...
            const stride = depth + 1;
            for (let x = 0; x < GEO.cols; x++) {
                if ((x + off + depth * 3) % stride !== 0) continue;
                const ch = fill[(x + off * 2 + depth * 5) % fill.length];
                if (ch === ' ') continue;
...
class Spawner {
    ...
    step(cells, score, speed) {
        ...
        if (this.nextHazard <= 0) {
            ...
            const combo = score > 700 && Math.random() < 0.18 && !HAZARDS[kind].air;
            if (combo) out.hazards.push(new Hazard('rateLimit', GEO.cols + 3 + 22));

            // Half the ground hazards get a token arc over them, claimed on the
            // next token beat.
            if (!HAZARDS[kind].air && !combo && Math.random() < 0.5) {
        ...
        if (this.nextToken <= 0) {
            ...
            if (this.arcOver !== null) {
                ...
                out.tokens.push(new Token(GEO.cols + 3, GEO.standRow - 1 - Math.floor(Math.random() * 3)));
            }
            this.nextToken = reach * (1.4 + Math.random() * 1.8);
        }
