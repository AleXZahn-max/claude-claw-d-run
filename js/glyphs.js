 ...
 * Skins
 *
 * A skin is Claw'd in a different phosphor. It is deliberately *not* a set of
 * hex codes handed to the renderer: each skin declares one quadruple per theme
 * and those get injected into both palettes as ordinary keys, so a skin is
 * addressed the same way every other colour in the game is. That is what keeps
 * the light theme working — a hard-coded #8FD86B crab is invisible on cream, but
 * `phosphorBase` simply resolves to a darker green when the palette swaps.
 *
 * Recolouring then costs one lookup per cell (see Terminal.recolor): the sprite
 * still says `claw`, and the map says `claw → phosphorBase`.
 * ------------------------------------------------------------------ */

const SKINS = [
    {
        id: 'coral', name: 'coral', note: 'as shipped',
        dark:  { base: '#D97757', deep: '#A9502F', hot: '#F6AE90', shell: '#E08A63' },
        light: { base: '#C35A32', deep: '#8A3D1E', hot: '#E07A4E', shell: '#B85028' },
    },
    {
        id: 'phosphor', name: 'phosphor', note: 'vt220 green',
        dark:  { base: '#8FD86B', deep: '#4E8C37', hot: '#C6F5A8', shell: '#6FBF4E' },
        light: { base: '#4C7A2E', deep: '#2E4F18', hot: '#6FA544', shell: '#3E6624' },
    },
    {
        id: 'amber', name: 'amber', note: 'wyse 50',
        dark:  { base: '#E5B25C', deep: '#9A6B12', hot: '#FBDDA0', shell: '#D19A3F' },
        light: { base: '#9A6B12', deep: '#6B4708', hot: '#C08A28', shell: '#835A0E' },
    },
    {
        id: 'ice', name: 'ice', note: 'cold cache',
        dark:  { base: '#5FC8D6', deep: '#2C6E7A', hot: '#B4EFF7', shell: '#43A7B6' },
        light: { base: '#1E7A88', deep: '#0F4A55', hot: '#3AA0AF', shell: '#166675' },
    },
    {
        id: 'plasma', name: 'plasma', note: 'off by one',
        dark:  { base: '#C08BE0', deep: '#7A4C9E', hot: '#E5C9F7', shell: '#A96FCC' },
        light: { base: '#7A4C9E', deep: '#52306E', hot: '#9A6ABF', shell: '#663D86' },
    },
    {
        id: 'ember', name: 'ember', note: 'on fire',
        dark:  { base: '#E06C75', deep: '#8E3D45', hot: '#F7B0B6', shell: '#C7545E' },
        light: { base: '#C0392F', deep: '#7E241C', hot: '#D9604F', shell: '#A32C24' },
    },
    {
        id: 'steel', name: 'steel', note: 'no colour, no bugs',
        dark:  { base: '#A9A19A', deep: '#5C554F', hot: '#E0D8D0', shell: '#8B837C' },
        light: { base: '#5C554F', deep: '#332E2A', hot: '#7E756D', shell: '#463F3A' },
    },
    {
        id: 'bone', name: 'bone', note: 'printed on paper',
        dark:  { base: '#EFE6DA', deep: '#9C9186', hot: '#FFFFFF', shell: '#D3C7B9' },
        light: { base: '#6B5F55', deep: '#3A302A', hot: '#8E8177', shell: '#544940' },
    },
];
...
for (const s of SKINS) {
    for (const mode of ['dark', 'light']) {
        const c = s[mode];
        PALETTES[mode][`${s.id}Base`] = c.base;
        PALETTES[mode][`${s.id}Deep`] = c.deep;
        PALETTES[mode][`${s.id}Hot`] = c.hot;
        PALETTES[mode][`${s.id}Shell`] = c.shell;
    }
}

const SKIN_IDS = SKINS.map((s) => s.id);

/** The key→key map a skin applies. Null for the default, so it costs nothing. */
function skinRecolor(id) {
    if (!id || id === 'coral' || !SKIN_IDS.includes(id)) return null;
    return { claw: `${id}Base`, clawDeep: `${id}Deep`, clawHot: `${id}Hot`, shell: `${id}Shell` };
}
...
function sprite(rows, mask = null, tint = 'fg') {
    ...
    /*
     * The columns the sprite's ink actually occupies. `blit(..., {solid:true})`
     * blanks that span on every row it draws, which is how a sprite becomes an
     * object you cannot see through.
     *
     * Sprites here have hollow interiors on purpose — that is what lets the legs,
     * the face and the snapped claws be separate layers of one crab — but it also
     * means whatever is behind them comes through the gaps. A ground strut landing
     * between `(x` and `x)` renders as `(x│ x)`, and that reads as a broken sprite
     * rather than as scenery.
     *
     * Ink columns rather than the full box, because a sprite pads itself out to a
     * rectangle and those pad columns are not part of it.
     */
    let left = w;
    let right = -1;
    for (const r of out.rows) {
        ...
        if (a < 0) continue;
        if (a < left) left = a;
        const b = r.trimEnd().length - 1;
        if (b > right) right = b;
    }
    out.span = right < 0 ? null : [left, right];
    return out;
}
...
const CLAWD = {
    ...
    /**
     * Four cells of crab, for a leaderboard row. It has to survive being one
     * line tall next to a name, so it keeps only what makes him recognisable:
     * two claws and two eyes. Tinted per player, which is what makes a board of
     * handles read as a board of crabs.
     */
    mini: sprite([R`}●●{`], [R`ceec`], 'claw'),
};
...
const GLYPHS = {
    ...
    SKINS, SKIN_IDS, skinRecolor, skinById,
