...
class ParticleManager {
    ...
    runDust(x, y, accent) {
        ...
        this.add(new GlyphParticle(
            x, y, -0.06 - Math.random() * 0.09, -0.02 - Math.random() * 0.04,
            ['·', '.', ' '], accent, 14 + Math.random() * 10,
        ));
    }
    ...
    landPuff(x, y, force) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            this.add(new GlyphParticle(
                ...
                dir * (0.1 + Math.random() * 0.22), -0.03 - Math.random() * 0.05,
                ['*', 'o', '·', '.'], 'dim', 12 + Math.random() * 10, 0.006,
            ));
        }
    }
    ...
    pickup(x, y) {
        for (let i = 0; i < 8; i++) {
            ...
            this.add(new GlyphParticle(
                ...
                ['◆', '◇', '·', '.'], i % 2 ? 'cyanHot' : 'cyan', 15 + Math.random() * 8,
            ));
        }
    }
    ...
    crash(x, y) {
        ...
        for (let i = 0; i < 46; i++) {
            ...
            const sp = 0.16 + Math.random() * 0.4;
            const ch = debris[Math.floor(Math.random() * debris.length)];
            this.add(new GlyphParticle(
                x + (Math.random() - 0.5) * 10, y - Math.random() * 4,
                Math.cos(a) * sp, Math.sin(a) * sp * 0.62,
                [ch, ch, '·', '.'], Math.random() < 0.55 ? 'bug' : 'clawDeep',
                26 + Math.random() * 26, 0.014,
            ));
        }
    }
    ...
    milestone(accent) {
        for (let i = 0; i < 22; i++) {
            const x = Math.random() * GEO.cols;
            this.add(new GlyphParticle(
                x, GEO.groundRow - Math.random() * 2,
                (Math.random() - 0.5) * 0.06, -0.09 - Math.random() * 0.1,
                ['✦', '*', '+', '·'], accent, 26 + Math.random() * 20,
            ));
        }
    }
    ...
    thinkBurst(x, y) {
        for (let i = 0; i < 26; i++) {
            ...
            this.add(new GlyphParticle(
                ...
                ['◆', '◇', '∘', '·'], i % 3 ? 'cyan' : 'cyanHot', 30 + Math.random() * 22,
            ));
        }
    }
}
